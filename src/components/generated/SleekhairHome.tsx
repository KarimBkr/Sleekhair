...
import { SleekhairCatalogue } from './SleekhairCatalogue';
export type SleekPage = 'home' | 'reservation' | 'admin' | 'membre' | 'catalogue';
export const SleekhairHome: React.FC = () => {
  const [page, setPage] = useState<SleekPage>('home');
  const [preselectedId, setPreselectedId] = useState<string | null>(null);
  const nav = (p: SleekPage, prestationId?: string) => {
    setPage(p);
    setPreselectedId(prestationId ?? null);
    window.scrollTo({
      top: 0,
      behavior: 'smooth'
    });
  ...
  if (page === 'reservation') return <SleekhairReservation onGoHome={() => nav('home')} preselectedPrestationId={preselectedId} />;
  if (page === 'catalogue') return <SleekhairCatalogue onNavigate={nav} />;
